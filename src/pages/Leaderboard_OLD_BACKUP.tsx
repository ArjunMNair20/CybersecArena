...
export default function Leaderboard() {
  ...
  const handleForceSyncClick = async () => {
    if (!user || !userScores || !state || !userProgress) {
      ...
      console.error('[Leaderboard] Missing data:', { user: !!user, userScores: !!userScores, state: !!state, userProgress: !!userProgress });
      return;
    }
    ...
    console.log('[Leaderboard] ');
    console.log('[Leaderboard] ╔═══ PROGRESS FROM APP STATE ═══╗');
    ...
    try {
      console.log('[Leaderboard] Calling syncUserScore...');
      ...
      // Force a slight delay then refresh
      await new Promise(resolve => setTimeout(resolve, 500));
      
      console.log('[Leaderboard] Loading fresh leaderboard data...');
      const data = await leaderboardService.getLeaderboard(100);
      setEntries(data);
      
      // Log what we got back
      const currentUserEntry = data.find(e => e.user_id === user.id);
      if (currentUserEntry) {
        console.log('[Leaderboard] ✅ Your data in leaderboard:', {
          ctf: currentUserEntry.ctf_solved_count,
          phish: currentUserEntry.phish_solved_count,
          code: currentUserEntry.code_solved_count,
          quiz: currentUserEntry.quiz_correct,
          score: currentUserEntry.total_score
        });
      } else {
        console.warn('[Leaderboard] ⚠️  Your entry not found in refreshed leaderboard');
      }
      
      setSyncStatus('✅ Leaderboard updated!');
